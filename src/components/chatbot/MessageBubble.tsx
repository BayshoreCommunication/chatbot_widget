...
const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const [selectedLocalSlot, setSelectedLocalSlot] =
  ...
  const parseAppointmentBookingLinks = (
    text: string
  ): { cleanText: string; bookingLinks: AppointmentBookingLink[] } => {
    const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g;
    ...
    const dateMatch = text.match(
      /(?:for\s+)?(\w+day),\s+([A-Za-z]+\s+\d+,\s+\d+)/i
    );
      ...
      if (line.includes("📅")) {
        ...
        if (timeMatch) {
          ...
          const dayMatch = line.match(
            /(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)(?:,\s+([A-Za-z]+\s+\d+))?/
          );
          const slotDay = dayMatch ? dayMatch[1] : currentDay;
          const slotDate = dayMatch && dayMatch[2] ? dayMatch[2] : currentDate;

          // Generate a unique ID if none exists
          const id = `slot_${slotDay}_${timeMatch[1]
            .replace(/\s+/g, "")
            .replace(":", "")}_${Math.floor(Math.random() * 1000)}`;
  ...
  const getIntroText = () => {
    if (!hasAppointmentInfo || !displaySlots || displaySlots.length === 0)
      return cleanText;
  ...
  const isVideoMessage = message.metadata?.type === "video";

  return (
    <div className={`flex ${isBot ? "justify-start" : "justify-end"}`}>
      {isBot && (
        <motion.div
          className="w-8 h-8 rounded-full bg-indigo-900 flex items-center justify-center mr-2 mt-1 overflow-hidden flex-shrink-0"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
      ...
      <div className="max-w-[100%] flex flex-col">
        ...
        >
                  ...
                  >
                    {displaySlots.map((slot, index) => (
                      <motion.button
                        ...
                        className={`px-3 py-2 bg-indigo-900 hover:bg-indigo-800 text-indigo-100 rounded text-sm transition-colors ${
                          selectedLocalSlot?.id === slot.id
                            ? "ring-2 ring-indigo-500 bg-indigo-800"
                            : ""
                        }`}
          ...
          <AnimatePresence>
            {isBot &&
              (selectedSlot || selectedLocalSlot) &&
              (awaitingConfirmation || selectedLocalSlot) &&
              !appointmentConfirmed && (
                <motion.div
                ...
                >
                  ...
                  >
                    <p className="font-medium">
                      {selectedSlot?.day || selectedLocalSlot?.day}
                    </p>
                    <p>{selectedSlot?.time || selectedLocalSlot?.time}</p>
                    <p className="text-xs text-gray-400">
                      ID: {selectedSlot?.id || selectedLocalSlot?.id}
                    </p>
                  </motion.div>
                  <motion.div
        ...
        <motion.span
          className={`text-xs mt-1 text-gray-400 ${
            isBot ? "ml-1" : "mr-1 self-end font-medium"
          }`}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
      ...
      {!isBot && (
        <motion.div
          className="w-8 h-8 rounded-full bg-indigo-600 border border-indigo-500 shadow-md flex items-center justify-center ml-2 mt-1 overflow-hidden flex-shrink-0"
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: "spring", stiffness: 500, delay: 0.1 }}
        >
